...
f.register(async (app) => {
  ...
  // Assign assessment to supplier (CTE upsert)
  ...
  // Submit responses (inline scoring; CTE upserts)
  app.post('/suppliers/:sid/responses', async (req) => {
    ...
    // 1) Upsert assessment_responses WITHOUT ON CONFLICT
    await pool.query(
      `WITH up AS (
    ...
    // 2) Fetch schema & compute weighted score
