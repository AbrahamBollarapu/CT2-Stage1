...
const okAuth = req =>
  AUTH_MODE === "none" ||
  (AUTH_MODE === "dev" && String(req.headers["x-api-key"] || "") === DEV_KEY);

function mergeInputs(url, body) {
  const q = Object.fromEntries(url.searchParams.entries());
  const out = {
    org_id: body.org_id || q.org_id || "demo",
    // accept legacy aliases: k -> meter, u -> unit
    meter:  body.meter  || q.meter  || body.k || q.k || null,
    unit:   body.unit   || q.unit   || body.u || q.u || "count",
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === "GET" && (eq(p, "/points") || eq(p, `${PREFIX}/points`))) {
    ...
    if (!inp.unit)  return sendJSON(res, 400, { ok: false, error: "unit (or u) required" });
    return sendJSON(res, 200, { ok: true, meter: inp.meter, unit: inp.unit, points: getPoints(inp) });
  }

  // **Compatibility**: treat GET on collection root as GET /points
  if (req.method === "GET" && (eq(p, "/") || eq(p, PREFIX))) {
    const inp = mergeInputs(url, {});
    if (!inp.meter) return sendJSON(res, 400, { ok: false, error: "use ?k=<meter>&u=<unit> (or meter/unit)" });
    return sendJSON(res, 200, { ok: true, meter: inp.meter, unit: inp.unit, points: getPoints(inp) });
  }
