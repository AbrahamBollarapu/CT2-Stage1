...
function ensureObjectForResponse(t) {
...
// UPSERT with tag sanitization: force non-object tags to '{}' before merging
    ...
    const sql = `
      ...
      on conflict (org_id, meter, unit, ts) do update
        set value = EXCLUDED.value,
            tags  = (
                      case when jsonb_typeof(points.tags) = 'object'
                           then points.tags else '{}'::jsonb end
                    )
                    ||
                    (
                      case when jsonb_typeof(EXCLUDED.tags) = 'object'
                           then EXCLUDED.tags else '{}'::jsonb end
                    )
    `;
  ...
  try {
    ...
    res.json({
      ...
      points: r.rows.map(x => ({
        ...
        tags: ensureObjectForResponse(x.tags)
      }))
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
