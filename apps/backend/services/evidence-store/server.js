// services/evidence-store/server.js
...
/**
 * POST /api/evidence
 * Upsert evidence metadata by sha256; returns the row UUID id.
 * Body: { org_id, sha256, size, mime, path }
 */
app.post("/api/evidence", async (req, res) => {
  ...
  if (!org_id || !sha256 || !size || !mime || !path) {
    return res
      .status(400)
      .json({ error: "org_id, sha256, size, mime, path required" });
  }
  try {
    const sql = `
...
/**
 * HEAD/GET /api/evidence/:id/content
 * Serve bytes by UUID id (resolves file path from DB).
 */
...
app.listen(process.env.PORT || 8000, () =>
  console.log("evidence-store listening")
);
